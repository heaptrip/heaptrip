...
import com.heaptrip.repository.account.helper.QueryHelper;
import com.heaptrip.repository.account.helper.QueryHelperFactory;
...
public class NotificationRepositoryImpl extends CrudRepositoryImpl<Notification> implements NotificationRepository {
	...
	public List<Notification> getNotificationsByCriteria(NotificationCriteria criteria) {
		QueryHelper<Criteria> queryHelper = QueryHelperFactory.getInstance(QueryHelperFactory.NOTIFICATION_HELPER);
		...
		Iterable<Notification> iter = coll.find(query, parameters).sort(sort).skip(skip).limit(limit)
				.hint(hint).as(Notification.class);
