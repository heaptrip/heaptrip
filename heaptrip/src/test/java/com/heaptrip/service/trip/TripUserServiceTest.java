...
public class TripUserServiceTest extends AbstractTestNGSpringContextTests {
	...
	public void addTableUser() {
		...
		tripUserService.addTableUser(TRIP_ID, TABLE_ITEM_ID, USER_ID);
		// check
		...
		List<TableMember> list = tripUserService.getTableMembers(TRIP_ID, TABLE_ITEM_ID);
	...
	public void removeTableUser() {
		...
		list = tripUserService.getTableMembers(TRIP_ID, TABLE_ITEM_ID);
		Assert.assertNotNull(list);
	...
	public void addTableInvite() {
		...
		tripUserService.addTableInvite(TRIP_ID, TABLE_ITEM_ID, USER_EMAIL);
		// check
		trip = tripRepository.findById(TRIP_ID);
		Assert.assertNotNull(trip);
		Assert.assertNotNull(trip.getTable());
	...
	public void addTableRequest() {
		...
		Assert.assertNotNull(trip.getTable()[0].getUsers());
		Assert.assertTrue(trip.getTable()[0].getUsers().equals(0L));
		tripUserService.addTableRequest(TRIP_ID, TABLE_ITEM_ID, USER_ID);
		// check
		trip = tripRepository.findById(TRIP_ID);
		Assert.assertNotNull(trip);
		Assert.assertNotNull(trip.getTable());
